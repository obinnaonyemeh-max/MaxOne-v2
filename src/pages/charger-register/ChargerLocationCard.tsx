...
const chargerMarkerIcon = new L.Icon({
...
interface ChargerLocationCardProps {
  location: { lat: number; lng: number }
  lastSeen: string
  lastPinged: string
}

export function ChargerLocationCard({
  location,
  lastSeen,
  lastPinged,
}: ChargerLocationCardProps) {
  return (
    <div className="bg-gray-25 border border-gray-200 rounded-lg overflow-hidden p-2">
      <div className="relative rounded-lg overflow-hidden h-full">
        ...
        <MapContainer
          ...
          zoom={14}
          style={{ height: "100%", width: "100%", borderRadius: "8px", minHeight: "220px" }}
          zoomControl={true}
        ...
        >
          ...
          />
          <Marker position={[location.lat, location.lng]} icon={chargerMarkerIcon} />
        </MapContainer>
      </div>
    </div>
  )
