...
const startedAtColumn: ColumnDef<RecoverySession> = {
  accessorKey: "startedAt",
  header: "Started",
  cell: ({ row }) => (
    <span className="font-medium text-table-text text-sm">{row.original.startedAt}</span>
  ),
}
...
const columnsByTab: Record<TabId, ColumnDef<RecoverySession>[]> = {
  "in-session": [...baseColumns, startedAtColumn, inSessionColumn, statusColumn],
        ...
        <div className="flex-1 flex flex-col min-h-0 rounded-t-[14px] rounded-b-[4px] border border-table-border">
          ...
          <div className="flex-1 overflow-y-auto">
            <DataTable
              ...
              onRowClick={(session) => navigate(`/portfolio/recovery/session/${session.id}`)}
            />
          </div>
        </div>
