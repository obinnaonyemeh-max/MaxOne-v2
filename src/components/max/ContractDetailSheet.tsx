...
export function ContractDetailSheet({ contract, isOpen, onClose, onUpdate }: ContractDetailSheetProps) {
  ...
  const metricsItems = [
    ...
    { label: "Total Amount Remitted", value: formatCurrency(contract.metrics.totalAmountRemitted) },
    ...
    <Sheet open={isOpen} onOpenChange={(open) => { if (!open) onClose() }}>
      <SheetContent size="lg" className="flex flex-col h-full max-w-[40vw]">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-5">
          <InfoCard title="Metrics Snapshot">
        ...
        <SheetFooter className="flex-wrap items-center justify-between gap-y-2">
          <Button variant="ghost" className="h-9 px-2 text-brand-dark" onClick={handleViewChampion}>
            See Champion Profile
          </Button>
          <div className="flex flex-wrap items-center justify-end gap-2">
            <Button variant="outline" className="h-9 px-3" onClick={() => setShowFullContract(true)}>
              View Full Contract
            </Button>
            {isPendingApproval && (
              <>
      ...
      <Dialog open={showFullContract} onOpenChange={setShowFullContract}>
        <DialogContent className="max-w-lg p-0">
          ...
          <DialogFooter className="px-6 py-4 border-t border-gray-100">
            <Button variant="outline" className="h-9" onClick={() => setShowFullContract(false)}>
              Close
            </Button>
          </DialogFooter>
        </DialogContent>
